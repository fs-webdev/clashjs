...
class ClashJS {
  ...
  _createAsteroids() {
    const numAsteroids = Math.floor(Math.pow(this._gameEnvironment.gridSize, 2) * 0.01)
  ...
  _createAsteroids2() {
    ...
    if (alivePlayers.length > 3) {
      ...
      for (let i = 0; i < numAsteroids; i++) {
        const adjoiningPositions = this._findAdjoiningPositions(alivePlayers[i].position)
        this._gameEnvironment.asteroids.push({
  ...
  nextPly() {
    ...
    if (this._currentPlayer === 0) {
      ...
      log('*** asteroids after detonate', JSON.stringify(this._gameEnvironment.asteroids))
  ...
  _findAdjoiningPositions(position) {
    ...
    return adjoining.filter(([x, y]) => x >= 0 && x < gridSize && y >= 0 && y < gridSize)
  }
